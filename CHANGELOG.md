...
### Fixed
- **Published dist now includes the StrictMode fix**: `0.2.1` was published before the `dist/` was rebuilt, shipping the old compiled output. This release re-publishes with a correctly built `dist/` that includes the `!unsubscribe` guard fix from `0.2.1`.
